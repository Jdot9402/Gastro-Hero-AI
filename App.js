...
const theme = {
  ...
  inset: { padding: 20 },          // använd som contentContainerStyle={theme.inset}
  space: (v)=> v*8,
  radius: { md:12 }
};
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    async function fetchFallbackMeals(){
      // 10 slumpade recept från TheMealDB (gratis fallback)
    ...
    async function fetchRecipes(){
      try{
        setLoading(true);
        // Primär källa: Spoonacular random
    ...
    <View style={{ flex:1, backgroundColor: theme.colors.background, paddingTop: 50 }}>
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <RecipeCard item={item} onPress={()=>{ setSelected(item); setShowModal(true); }} />
          )}
      ...
      {/* Detaljvy som modal (all info i appen) */}
      <Modal
        visible={showModal}
        animationType="slide"
        transparent
      ...
      >
        <View style={{ flex:1, backgroundColor:'rgba(0,0,0,0.6)', justifyContent:'flex-end' }}>
          <View style={{ backgroundColor: theme.colors.surface, borderTopLeftRadius:16, borderTopRightRadius:16, maxHeight:'85%' }}>
            <ScrollView contentContainerStyle={theme.inset}>
              <Text style={{ color: theme.colors.text, fontWeight:'800', fontSize:20 }}>
                {selected?.title || ''}
              </Text>
              <Text style={{ color: theme.colors.textMuted, marginTop:4 }}>
                {selected?.minutes ? `${selected.minutes} min · `: ''}{selected?.cuisine || 'world'}
              </Text>
              ...
              {!!selected?.ingredients?.length && (
                <View style={{ marginTop: theme.space(4) }}>
                  <Text style={{ color: theme.colors.text, fontWeight:'700', fontSize:16, marginBottom: 6 }}>
                    Ingredienser
                  </Text>
              ...
              <View style={{ flexDirection:'row', gap:12, justifyContent:'center', marginTop: theme.space(3), marginBottom: theme.space(2) }}>
                ...
                <Pressable
                  ...
                  style={{ backgroundColor: theme.colors.accent, paddingVertical: theme.space(3), paddingHorizontal: theme.space(6), borderRadius: 999 }}
                >
